...
def pred_vs_coder(num_robots):
    ...
    predictions_heart = np.array([coder_evaluations.get(t) for t in common_times_heart])
    ...
    predictions_breat = np.array([coder_evaluations.get(t) for t in common_times_breat])
    ...
    print(predictions_postu)
    print(observations_postu)
    ...
    predictions_activ = np.array([coder_evaluations.get(t) for t in common_times_activ])
    ...
    predictions_ecgAm = np.array([coder_evaluations.get(t) for t in common_times_ecgAm])
    ...
    predictions = np.array([coder_evaluations.get(t) for t in common_times])
...
pred_vs_coder(3)
ext = "eps"
plt.savefig("coder_vs_physiology.{}".format(ext), format=ext)
plt.show()
