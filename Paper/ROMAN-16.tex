...
Here, the horizontal axis denotes $\hat{s}$ which the robot's cognitive load estimation or CLE of human operator by the robot. The vertical axis denotes our observed $\hat{o}$ of human operator by a human observer. We have sampled both $\hat{s}$ and $\hat{o}$ at every 12 seconds and took the average of all the users CLE and observed stress level respectively. The task complexity was increased by the magnitude of one additional robot; as shown in Fig.~\ref{fig:final-results}~\subref{fig:final-results_a} and ~\ref{fig:final-results}~\subref{fig:final-results_b} respectively. The graph plots the average CLE of all the human operator participated in the game by the robot and our observation of the human operator. It is critical for interpreting the
...
appropriate to compare. Thus the cross correlation between the $\hat{s}$ and $\hat{o}$ can tell us how our model calculated the CLE with respect to actual observation of human operator by a human observer. Fig.\ref{fig:final-results} shows that this model has a 18.14\% and 42.00\% correlation with actual observed CLE for experiment with one robot and two robots respectively. Whenever cognitive stress occurs or changes,
...
\begin{figure}  
\centering
\subfloat[One robot]{\label{fig:final-results_a}\includegraphics[width=.5\textwidth]{CLE_reg_One_robot.eps}}\\
\subfloat[Two robots]{\label{fig:final-results_b}\includegraphics[width=.5\textwidth]{CLE_reg_One_robot.eps}}	
...
% ********* 12 sec interval graph. not using this ************
%Analysis of the results from the coin game experiment are in agreement
%with the original hypothesis. In general, the robot correctly predicts
%the cognitive load that its operator was under in every
%scenario. These findings can be seen in Fig.~\ref{fig:final-results}.
%Here, the horizontal axis denotes time in second the coin game was played and also denotes an increase in task complexity over time;
%the user must issue an increasing number of commands and collect coins
%in an increasingly short duration. The task complexity was also increased by the magnitude of one additional robot. The graph plots the average CLE of all the human operator participated in the game by the robot and our observation of the human operator. It is critical for interpreting the
...
%where the actual tasks they are being asked to perform are opaque to
%the robot.
% ********* 12 sec interval graph. not using this END ************
