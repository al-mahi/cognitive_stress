...
question of how the robot should differentiate between trustworthy and
untrustworthy user input.  The robot's trust model is concerned with
three features that capitalized on the information gathered from both
limited sensing capabilities and user feedback.  These are:
\textit{disparity}, \textit{percent damage} and \textit{goal arrival
  delay}:
...
\begin{description}
  ...
  \begin{equation}
  \label{eq:disparity}
  \text{disparity}(i)=\sqrt{\left(\text{xexp}_i-\text{xusr}_i\right){}^2+\left(\text{yexp}_i-\text{yusr}_i\right){}^2}
  \end{equation}
  where $xep_i$, $yexp_i$ and $xusr_i$, $yusr_i$ are the $x$ and $y$
  components at the $i^{th}$ goal for the point expected by the robot
  and given by the user respectively.
  ...
  \text{damage}(i)=\frac{\text{h}_{\text{0}}-\text{h}_i}{\text{h}_{\text{0}}}
  \end{equation}
  where $h_0$ is the initial health and $h_i$ the health upon reaching goal $i$.
 
  \item[Time Delay] is the difference between when the robot would
    expect to reach a goal position if it was navigating autonomously
    and the time taken when it is assisted by the human operator.
  \begin{equation}
  \label{eq:time_delay}
  \text{delay}(i)=t_{\text{robot}}-t_i
  \end{equation}
  where $t_{robot}$ is the time the robot would arrive at goal $i$
  navigating autonomously and $t_{i}$ is the actual arrival time when
  aided by the user.
\end{description}

We can combine these metrics in such away that a robot can to evaluate
...
maze game.
  \begin{equation}
  \label{eq:mistrust}
  \mu(i) = \text{disparity}(i) + \text{delay}(i) + \text{damage}(i)
  \end{equation}
...
trustworthiness falls in the range $0$ to $1$.  We can take the
normalized value for all the metrics defined as follows instead of
Eq.~\ref{eq:disparity},~\ref{eq:health_calc} and ~\ref{eq:time_delay}
respectively:

\begin{equation}
  \label{eq:disparity_norm}
  d_{i} = \dfrac{disparity_{i}}{max(disparity) - min(disparity) }
\end{equation}
 
\begin{equation}
  \label{eq:health_calc_norm}
  h_{i} = \dfrac{damage_{i}}{max(damage) - min(damage) }
\end{equation}

\begin{equation}
  \label{eq:time_delay_norm}
  \Delta t_{i} = \dfrac{delay_{i}}{max(delay) - min(delay) }
\end{equation}

Now we can also describe our trustworthiness function as:

  \begin{equation}
  \label{eq:trust}
  \tau'(i) = 1 - (disparity_{i} + delay_{i} + damage_{i})
  \end{equation}

However, we also want the trustworthiness function to be normalized so
that we can manipulate this data effectively while interpreting this
data. Finally, our hypothesis regarding about trustworthiness of human
operator is the function:

\begin{equation}
  \label{eq:trust_norm}
  \tau(i) = \dfrac{\tau'(i)}{max(\tau'(i)) - min(\tau'(i)) } , 0\leq i\leq 31
\end{equation}

We can also define a decision function $\delta$ based on the trust
value. The decision function decides whether an human assistance is
trustworthy or not based on $\tau$ and a threshold $\theta$.

\[
\delta(i) = 
\begin{cases}
True, & \text{if } \tau(i)\ge \theta\\
False, & Otherwise\\
\end{cases}
\]
