...
double distanceToPointFromPoint(geometry_msgs::Point p1, geometry_msgs::Point p2) {
...
void GameModel::startRobot(std::string robotName) {
    ...
    while (!ac.waitForServer(ros::Duration(5.0)) && connectionAttempts < 5) {
        ROS_INFO("Waiting for move base action server... Attempts: %i", connectionAttempts);
        connectionAttempts++;
    }
...
//		ROS_INFO("Checking robot %s", entry.first.c_str());
        ...
        if (entry.second.getCurState() == RobotState::MOVING_TO_START ||
            entry.second.getCurState() == RobotState::RUNNING_MAZE) {
...
void GameModel::submitAndUpdateAverageScores(std::string name,
                                             std::vector<double> times,
                                             std::vector<int32_t> healthPoints,
    ...
    this->runDataVector.push_back(
            RunData(name, false, times, healthPoints, percentDamage, givenPoints, expectedPoints, disparity, inputDelay,
                    trustScores, goalFrequencyForPoints, timeRemainingAtGoal, estimatedGoalIndexForGoal,
                    //--------added by Mahi------------------------
                    clicksAtGoals, mouseDistAtGoal, ignorance_time));
    map_robot_clicks[name] = 0;
    map_robot_moves[name] = 0;
    //---------------------------------------------

    //Compute average duration
    this->averageRunTimesVector.push_back(ros::Duration(times.back() - times.front()));
    this->averageRunTime = ros::Duration(
            (this->averageRunTime + this->averageRunTimesVector.back()).toSec() / this->averageRunTimesVector.size());
...
void GameModel::computeValuesForDataSample(GameModel::RunData &data) {
    ...
    for (int i = data.disparity.size() - 1; i >= 0; i--) {
        ...
        data.percentDamage[i] =
                (double(data.healthAtGoal[0]) - double(data.healthAtGoal[i])) / double(data.healthAtGoal[0]);
        printf("Values for percent damage: @ 0 = %.3f\t@%i = %.3f\n", double(data.healthAtGoal[0]), i,
               double(data.healthAtGoal[i]));
        printf("Comp. percentDamage @ %i: %.6f\n", i, data.percentDamage[i]);

        //Compute trust score (unnormalized trust value)
//		data.trustScores[i] = data.disparity[i].z + data.percentDamage[i] + data.timeRemainingAtGoal[i];
        data.trustScores[i] =
                (this->kPercentDamage * data.percentDamage[i]) - (this->kDispairty * data.disparity[i].z) -
                (this->kTimeRemaining * (120.0 - data.timeRemainingAtGoal[i]));
...
bool GameModel::exportRunData(std::string robotName) {
    ...
    for (RunData data : toExport) {
        ...
        std::string csvToExport = "Robot,Runtime,Run Before,Arrival Time,Health at Goal,Percent Damage, Estimated Goal Index, Given x,Given y,Expected x,Expected y,Disparity, Time Remaining At Goal,"
                //-----added by Mahi------
                "Mouse Clicks,Mouse Movements, Ignorance Time,"
                //------------------------
...
//		}
        ...
        for (int i = 0; i < data.trustScores.size(); i++) {
            ...
            curRow.append(((data.estimatedGoalIndexForGoal[i] == INT32_MIN) ? "*," : (
                    std::to_string(data.estimatedGoalIndexForGoal[i]) + ",")));
            ...
            if (data.timeRemainingAtGoal[i] == INT32_MIN) {
                curRow.append("*,");
            } else {
                curRow.append(std::to_string(data.timeRemainingAtGoal[i]) + ",");
            }
            //------------added by Mahi--------------------------------------
            curRow.append(std::to_string(data.clicksAtGoals[i]) + ",");
            curRow.append(std::to_string(data.mouseDistAtGoal[i]) + ",");
            curRow.append(std::to_string(data.ignorance_time[i]) + ",");
            //---------------------------------------------------------------
...
GameModel::GoalAndIndexQuery *GameModel::getClosestGoalAndPathIndex(geometry_msgs::Point goal, int curGoalIndex) {

    geometry_msgs::Point curClosest = this->expectedPoints.front();
    double shortestPath = 1000;
    double curPathLength;

    int goalIndex;

//	ROS_INFO("GameModel will ignore index %i", curGoalIndex);

    for (int i = 0; i < this->expectedPoints.size(); i++) {

        curPathLength = distanceToPointFromPoint(goal, this->expectedPoints[i]);

        if (curPathLength < shortestPath && i != curGoalIndex) {
//			ROS_INFO("%i: new closest: %.6f < %.6f\n", i, curPathLength, shortestPath);

            curClosest = this->expectedPoints[i];
            shortestPath = curPathLength;
            goalIndex = i;
        }
    }

//	ROS_INFO("%i\tClosest (%.6f, %.6f)\n", goalIndex, curClosest.x, curClosest.y);

    return new GoalAndIndexQuery(goalIndex, curClosest);
}
